...
function convertPath (path) {
  ...
  path = compile(path)(parameters).replace(/%7B/g, '{').replace(/%7D/g, '}')
...
function generateParameters (parameterTypes, groupOrAction) {
  ...
  parameterTypes.forEach(parameterType => {
    ...
    if (groupOrAction instanceof Group) {
      ...
      while (true) {
        if (parent[parameterType]) Object.assign(groupParameters, parent[parameterType])
        if (!parent.parent) break
        parent = parent.parent
      }
    }
    if (groupOrAction instanceof Group ? Object.keys(groupParameters).length : groupOrAction[parameterType].length) {
      const parameters = groupOrAction instanceof Group ? groupParameters : groupOrAction[parameterType][0]
      let parameter
      if (parameterType === 'requestBodies') {
        ...
        for (const key of Object.keys(parameters)) {
          parameter = Object.assign(jsonSchemaFromObject(parameters[key], false), {
            name: key,
            in: { parameters: 'path', queries: 'query', requestHeaders: 'header' }[parameterType]
          })
...
function jsonSchemaFromObject (object, splitDescriptions, uncaptured, key, parentSchema) {
  const schema = {}
  ...
  if (value === null || value === undefined) {
    ...
    schema.type = 'array'
    if (value.length) schema.items = jsonSchemaFromObject(object[0], splitDescriptions, value[0], 0, schema)
  } else {
    schema.type = 'object'
    ...
    if (keys.length) {
      ...
      for (const key of Object.keys(value)) {
        schema.properties[key] = jsonSchemaFromObject(object[key], splitDescriptions, value[key], key, schema)
      }
    }
  }
...
function generate (group, options = {}) {
  ...
  if (group.docs.descriptions.length) swagger.info.description = group.docs.descriptions.join('\n\n')
  ...
  while (groupStack.length) {
    ...
    if (group.docs.title && group.actions.length) {
      ...
      const tagIndex = swagger.tags.findIndex(tag => tag.name === group.docs.title)
      if (tagIndex >= 0) {
        ...
        if (group.docs.descriptions.length) tag.description = group.docs.descriptions.join('\n\n')
    ...
    for (const action of group.actions) {
      ...
      if (swagger.basePath) url = url.replace(pathToRegexp(swagger.basePath, { end: false }), '')
      url = convertPath(url)
      if (!swagger.paths[url]) swagger.paths[url] = {}

      const method = action.docs.method.toLowerCase()
      if (swagger.paths[url][method]) { // combine title and description for duplicated operations
        const operation = swagger.paths[url][method]

        if (group.docs.title) {
          if (!operation.tags) operation.tags = []
          if (!operation.tags.includes(group.docs.title)) operation.tags.push(group.docs.title)
        }
        ...
        if (action.docs.descriptions.length) operation.description = action.docs.descriptions.join('\n\n')

        const parameters = generateParameters(['requestHeaders', 'parameters', 'queries'], group)
          .concat(generateParameters(['requestHeaders', 'parameters', 'queries', 'requestBodies'], action))
        if (parameters.length) operation.parameters = parameters

        const cycle = Math.max(action.responseHeaders.length, action.statusCodes.length, action.responseBodies.length)
        for (let i = 0; i < cycle; ++i) {
          if (action.responseBodies[i] || action.statusCodes[i] || action.responseHeaders[i]) {
            const statusCode = action.statusCodes[i] || 200
            let contentType = 'application/json'
            if (operation.responses[statusCode]) continue
            if (action.responseHeaders[i] && action.responseHeaders[i]['content-type']) {
              contentType = action.responseHeaders[i]['content-type'].split(';')[0]
            }
            ...
            if (action.responseBodies[i]) {
              ...
              if (contentType === 'application/json') {
                response.schema = jsonSchemaFromObject(action.responseBodies[i], true)
                response.examples = {
            ...
            if (action.responseHeaders[i]) {
              ...
              for (const headerName of Object.keys(action.responseHeaders[i])) {
                response.headers[headerName] = jsonSchemaFromObject(action.responseHeaders[i][headerName], false)
              }
            }
  ...
  return options.useYaml ? yaml.safeDump(swagger, {
