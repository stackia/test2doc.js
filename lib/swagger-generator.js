  ...
  while (groupStack.length) {
    ...
    for (const action of group.actions) {
      ...
      if (swagger.paths[url][method]) { // combine title and description for duplicated operations
        ...
        const cycle = Math.max(action.responseHeaders.length, action.responseBodies.length)
        for (let i = 0; i < cycle; ++i) {
          if (action.responseBodies[i] || action.responseHeaders[i]) {
            let statusCode = 200
            let contentType = 'application/json'
            let docs
            if (action.responseBodies[i]) {
              docs = capture.docs(action.responseBodies[i])
              if (docs.statusCode) statusCode = docs.statusCode
            }
            if (operation.responses[statusCode]) continue
            if (action.responseHeaders[i] && action.responseHeaders[i]['content-type']) {
              contentType = action.responseHeaders[i]['content-type']
            }
            const response = {}
            if (action.responseBodies[i]) {
