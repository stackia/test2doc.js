...
function generateParameters (parameterTypes, groupOrAction) {
  ...
  parameterTypes.forEach(parameterType => {
    if (parameterType.array ? groupOrAction[parameterType.name].length : groupOrAction[parameterType.name]) {
      const parameters = parameterType.array ? groupOrAction[parameterType.name][0] : groupOrAction[parameterType.name]
      let parameter
      if (parameterType.name === 'requestBodies') {
        parameter = {
          name: 'body',
          in: 'body',
        ...
        for (const key of Object.keys(parameters)) {
          parameter = Object.assign(jsonSchemaFromObject(parameters[key], false), {
            name: key,
            in: { parameters: 'path', queries: 'query' }[parameterType.name]
          })
          if (parameterType.name === 'parameters') parameter.required = true
        }
      }
  ...
  while (groupStack.length) {
    ...
    for (const action of group.actions) {
      ...
      if (swagger.paths[url][method]) { // combine title and description for duplicated operations
        ...
        const parameters = generateParameters([{ name: 'parameters' }, { name: 'queries' }], group)
          .concat(generateParameters([{ name: 'parameters', array: true }, { name: 'queries', array: true }, { name: 'requestBodies', array: true }], action))
        if (parameters.length) operation.parameters = parameters

        for (const responseBody of action.responseBodies) {
          const docs = capture.docs(responseBody)
          const statusCode = docs.statusCode || 200
          if (operation.responses[statusCode]) continue
          operation.responses[statusCode] = {
            description: docs.descriptions.join('\n\n'),
            schema: jsonSchemaFromObject(responseBody, true),
            examples: {
              'application/json': capture.undo(responseBody, true)
            }
          }
        }
      }
