...
class Group {
  ...
  group (title) {
    let groupIndex = this.children.findIndex(g => g.docs.title === title)
  ...
  action (title) {
    let actionIndex = this.actions.findIndex(a => a.docs.title === title)
    if (actionIndex >= 0) {
  ...
  emit (file, generator = 'apib', options = {}) {
    if (typeof generator === 'string') generator = require(`./${generator}-generator`)
