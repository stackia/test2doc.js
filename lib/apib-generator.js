...
function objectToMson (object, rootName = '', indentLevel = 0, extraNewline = true, ignoreRootType = false) {
  ...
  capture.traverse(object, node => {
    ...
    if (node.index !== undefined) {
      const parentDocs = capture.docs(node.parent.value)
      if (parentDocs.offset !== undefined || parentDocs.limit !== undefined) {
        const offset = parentDocs.offset || 0
        const limit = parentDocs.limit !== undefined ? parentDocs.limit : Infinity
        if (node.index < offset || node.index >= offset + limit) {
          node.ignored = true
          return
        }
      }
