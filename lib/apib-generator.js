...
function generate (group) {
  ...
  while (groupStack.length) {
    ...
    if (group.depth === 0) {
      document += 'FORMAT: 1A9\n'
      if (group.docs.host) {
        var h_path = group.docs.basePath ? `${group.docs.host}/${group.docs.basePath}` : group.docs.host
        document += `HOST: ${group.docs.schemes[0] || 'http'}://${h_path}\n`
      }
      document += '\n'
      if (group.docs.title) {
      ...
      if ((group.children.length || group.actions.length)) { // is Resource Group
        ...
        if (!title && (!group.docs.basePath || group.docs.version)) title = 'Untitled Resource'
        if (title) {
          document += title
          if (group.docs.version) document += ` ${group.docs.version}`
    ...
    for (const action of group.actions) {
      ...
      for (let i = 0; i < cycle; ++i) {
        ...
        // Action response
        if (action.responseHeaders[i] || action.statusCodes[i] || action.responseBodies[i]) {
          let contentType = 'application/json'
          if (action.responseHeaders[i] && action.responseHeaders[i]['content-type']) contentType = action.responseHeaders[i]['content-type'].split(';')[0]
          document += `+ Response ${action.statusCodes[i] || 200} (${contentType})\n\n`
