...
function generateHeaders (headers, indentLevel = 0) {
  ...
  for (const headerName of Object.keys(headers)) {
    const values = capture.typeOf(headers[headerName]) === 'array' ? headers[headerName] : [headers[headerName]]
...
function generate (group) {
  ...
  while (groupStack.length) {
    ...
    while (true) {
      ...
      if (parent.requestHeaders) Object.assign(groupRequestHeaders, parent.requestHeaders)
    ...
    if (group.depth === 0) {
      ...
      if (group.depth === 1 && group.children.length) { // is Resource Group
        ...
        if (!title && (!path || group.docs.version)) title = 'Untitled Resource'
        if (title) {
          document += title
          if (group.docs.version) document += ` ${group.docs.version}`
        ...
        // Resource parameters
        if (Object.keys(groupParameters).length || Object.keys(groupQueries).length) document += generateDocForParameters(Object.assign({}, groupParameters, groupQueries))
      }
    }
    ...
    for (const action of group.actions) {
      ...
      const cycle = Math.max(action.parameters.length, action.queries.length, action.requestHeaders.length, hasGroupHeaders ? 1 : 0, action.requestBodies.length, action.responseHeaders.length, action.responseBodies.length)
      for (let i = 0; i < cycle; ++i) {
        ...
        // Action request
        if (hasGroupHeaders || action.requestHeaders[i] || action.requestBodies[i] || (i > 0 && hasParameters)) {
          document += '+ Request'
          const docs = capture.docs(action.requestBodies[i])
          if (docs.descriptions[0]) document += ` ${docs.descriptions[0]}`
          let contentType = 'application/json'
          if (action.requestHeaders[i] && action.requestHeaders[i]['content-type']) {
            contentType = action.requestHeaders[i]['content-type']
          }
          document += ` (${contentType})\n\n`
          if (hasGroupHeaders || action.requestHeaders[i]) {
            document += generateHeaders(Object.assign({}, groupRequestHeaders || {}, action.requestHeaders[i]), 1)
          }
          if (i > 0 && hasParameters) {
        ...
        // Action response
        if (action.responseHeaders[i] || action.responseBodies[i]) {
          const docs = capture.docs(action.responseBodies[i])
          let contentType = 'application/json'
          if (action.responseHeaders[i] && action.responseHeaders[i]['content-type']) contentType = action.responseHeaders[i]['content-type']
          document += `+ Response ${docs.statusCode || 200} (${contentType})\n\n`
