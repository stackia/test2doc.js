const { symbolDescription: symbolDesc } = require('symbol-description')
...
function traverse(object, visitFn, map = false, inplaceUpdate = true) {
  function childrenOf(node) {
    ...
    if (node.valueType === 'object' && node.value) {
      ...
      children = sliceArray(node.value, nodeDocs.offset, nodeDocs.limit).map(
        (item, index) => {
          return {
            index,
            value: item,
  ...
  while (stack.length) {
    ...
    if (map) {
      ...
      if (newObjectType === 'object' && newObject) newObject = { ...newObject }
...
function capture(object, path, contextToMerge) {
  ...
  if (contextToMerge) {
    ...
    for (const property of Object.keys(contextToMerge.propertyProxies)) {
      const propertyContext =
        contextToMerge.propertyProxies[property][contextSymbol]
      context.propertyProxies[property] = capture(
  ...
  // eslint-disable-next-line no-param-reassign
  object = wrap(object)
  const docDescriptor = Object.assign(
